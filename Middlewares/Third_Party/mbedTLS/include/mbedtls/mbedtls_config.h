...
/* v3.x split the old MBEDTLS_SSL_MAX_CONTENT_LEN into separate in/out buffers.
 * Default is 16384 each - too big for your RAM budget. HiveMQ's handshake
 * (cert chain etc.) fits comfortably in 4K in; keep out smaller since your
 * PUBLISH payloads are tiny JSON strings. */
...
/* Bare-metal target - no /dev/urandom or CryptGenRandom to fall back on.
 * Without this, entropy_poll.c #errors out at compile time. Hardware RNG
 * via MBEDTLS_ENTROPY_HARDWARE_ALT above is the only entropy source used. */
#define MBEDTLS_NO_PLATFORM_ENTROPY
...
/* No RTC synced to real epoch time on this board (no NTP client running).
 * MBEDTLS_HAVE_TIME gates mbedtls_ms_time() (which platform_util.c can't
 * implement on bare-metal without a POSIX/Win32 clock - hence the build
 * error) AND X.509 cert expiry checking. Disabling it skips expiry
 * validation (common, accepted tradeoff on RTC-less embedded targets) -
 * you're still getting full chain-of-trust + hostname verification, just
 * not "is this cert currently within its validity window". If you later
 * sync real time via SNTP and want expiry checks back, re-enable this and
 * implement MBEDTLS_PLATFORM_MS_TIME_ALT backed by the RTC instead. */
#undef MBEDTLS_HAVE_TIME
#undef MBEDTLS_HAVE_TIME_DATE

/* mbedTLS's own POSIX/Windows BSD-socket helper module - only relevant if you
 * call mbedtls_net_connect() etc. You don't: tls_transport.c bridges mbedTLS
 * to the ISM43362 via mbedtls_ssl_set_bio() instead. Assumes Unix/Windows
 * headers (sys/socket.h) that don't exist on bare-metal ARM, so it must be off. */
#undef MBEDTLS_NET_C

/* mbedTLS's own OS-based timer utility, used for DTLS retransmission delays
 * (mbedtls_timing_set_delay/get_delay). You're doing TLS over TCP, not DTLS,
 * and your own timeouts live in tls_transport.c's BIO callbacks - unused. */
#undef MBEDTLS_TIMING_C

/* Filesystem-based cert loading (mbedtls_x509_crt_parse_path/_file), needs
 * dirent.h/fopen. No filesystem exists on this MCU - you load the CA cert
 * from ca_cert_pem[] in flash via mbedtls_x509_crt_parse() instead. */
#undef MBEDTLS_FS_IO

/* PSA's file-backed secure key storage - depends on MBEDTLS_FS_IO (just
 * disabled above), and irrelevant since you're not using PSA crypto at all. */
#undef MBEDTLS_PSA_ITS_FILE_C

/* Pulls in about 15 psa_crypto_x.c files needing psa headers that aren't
 * present as static files in a plain git clone (mbedTLS generates some of
 * these via build scripts you're not running). You're using the classic
 * mbedtls_ssl_x / mbedtls_x509_x / mbedtls_pk_x APIs throughout
 * tls_transport.c, never the PSA equivalents, so this whole subsystem is
 * dead weight. */
#undef MBEDTLS_PSA_CRYPTO_C

/* Both default-on and both require MBEDTLS_PSA_CRYPTO_C, which is now off:
 * LMS_C is a post-quantum signature scheme unrelated to a TLS client, and
 * PSA_CRYPTO_STORAGE_C is PSA's persistent key storage - same dead subsystem. */
#undef MBEDTLS_LMS_C
#undef MBEDTLS_PSA_CRYPTO_STORAGE_C

/* GCC 14.x's register allocator chokes on bn_mul.h's hand-rolled Thumb-2
 * MULADDC_X8 asm macro at -O0 ("impossible constraints or not enough
 * registers"). Force the portable C bignum multiply instead - negligible
 * cost for a single TLS handshake. */
